...
wahrbetasq<-solve(covg[2:(p+1),2:(p+1)],covg[2:(p+1),1])^2*diag(covg[2:(p+1),2:(p+1)])/variances[[1]]
#normalize
if (rela) wahrbetasq<-wahrbetasq/sum(wahrbetasq) 

#ranking 
raengebetasq<-p+1-rank(wahrbetasq)
#pairwise differences
if (diff & p>2)
diffbetasq<-wahrbetasq[nchoosek(p,2)[1,]]-wahrbetasq[nchoosek(p,2)[2,]]
if (diff & p==2)
diffbetasq<-wahrbetasq[1]-wahrbetasq[2]
slot(ausgabe,"betasq")<-wahrbetasq
if (rank) slot(ausgabe,"betasq.rank")<-raengebetasq
if (diff) slot(ausgabe,"betasq.diff")<-diffbetasq
return(ausgabe)
}
