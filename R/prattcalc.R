...
wahrpratt<-solve(covg[2:(p+1),2:(p+1)],covg[2:(p+1),1])*covg[2:(p+1),1]/covg[1,1]
#normalize
if (rela) wahrpratt<-wahrpratt/sum(wahrpratt)

#ranking
raengepratt <- p+1-rank(wahrpratt)
#pairwise differences
if (diff & p>2) diffpratt <- wahrpratt[nchoosek(p,2)[1,]]-wahrpratt[nchoosek(p,2)[2,]]
if (diff & p==2) diffpratt <- wahrpratt[1]-wahrpratt[2]
slot(ausgabe,"pratt")<-wahrpratt
if (rank) slot(ausgabe,"pratt.rank")<-raengepratt
if (diff) slot(ausgabe,"pratt.diff")<-diffpratt
return(ausgabe)
}
