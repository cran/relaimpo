...
function (x, ..., names.abbrev = 4) 
{
    ...
    if (length(type) == 0) 
        ...
        if (ntype > 4) 
            par(mfrow = c(2, 3))
        if ("lmg" %in% x@type) 
            barplot(x@lmg, ylab = ylab, main = "Method LMG", 
                names.arg = xnames, ylim = c(axmin, axmax))
        if ("pmvd" %in% x@type) 
            barplot(x@pmvd, ylab = ylab, main = "Method PMVD", 
                names.arg = xnames, ylim = c(axmin, axmax))
        if ("last" %in% x@type) 
            barplot(x@last, ylab = ylab, main = "Method Last", 
                names.arg = xnames, ylim = c(axmin, axmax))
        if ("first" %in% x@type) 
            barplot(x@first, ylab = ylab, main = "Method First", 
                names.arg = xnames, ylim = c(axmin, axmax))
        if ("betasq" %in% x@type) 
            barplot(x@betasq, ylab = ylab, main = "Method Betasq", 
                names.arg = xnames, ylim = c(axmin, axmax))
        if ("pratt" %in% x@type) 
            barplot(x@pratt, ylab = ylab, main = "Method Pratt", 
                names.arg = xnames, ylim = c(axmin, axmax))
        par(op)
    }
}
