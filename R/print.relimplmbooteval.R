...
function(x,...){
if(!(is(x,"relimplmbooteval"))) stop("x must be the output from function calc.relimp")
    ### offset besser bestimmen
    p<-length(slot(x,"namen"))-1
    nlev<-length(x@level)
    print.relimplm(x)
    cat("\n","\n","Confidence interval information (", x@nboot, "bootstrap replicates, bty=",x@bty,"):","\n")    
    offset<-max(nchar(rownames(x@mark)))+12
    if (x@sort) cat("Sorted Relative Contributions with confidence intervals:","\n","\n")
    else cat("Relative Contributions with confidence intervals:","\n","\n")
    if (x@rank) 
    {
    cat(paste(rep(" ",offset+nlev*(p+1)),collapse=""),
    "Lower",paste(rep(" ",7*nlev-5),collapse=""),"Upper","\n",sep="")}
    else 
    {
    cat(paste(rep(" ",offset),collapse=""),
    "Lower",paste(rep(" ",7*nlev-5),collapse=""),"Upper","\n",sep="")
    }
