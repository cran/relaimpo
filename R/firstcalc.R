...
#normalize
if (rela) wahr1st<-wahr1st/sum(wahr1st) else wahr1st<-wahr1st/(variances[[1]])

#ranking
raenge1st<-p+1-rank(wahr1st)
#pairwise differences
if (diff & p>2)
diff1st<-wahr1st[nchoosek(p,2)[1,]]-wahr1st[nchoosek(p,2)[2,]]
if (diff & p==2)
diff1st<-wahr1st[1]-wahr1st[2]
slot(ausgabe,"first")<-wahr1st
if (rank) slot(ausgabe,"first.rank")<-raenge1st
if (diff) slot(ausgabe,"first.diff")<-diff1st
return(ausgabe)
}
